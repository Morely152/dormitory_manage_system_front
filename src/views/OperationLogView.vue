...
async function loadLogs() {
  ...
  try {
    const result = unwrapPage(
      await getOperationLogs({ page: pagination.page, size: pagination.size }),
      '系统操作日志加载失败',
    )
    ...
    if (currentVersion === requestVersion) {
      ElMessage.error(requestErrorMessage(error, '系统操作日志加载失败'))
    }
  } finally {
  ...
  <main class="log-page">
    ...
    <section class="log-page__workspace" aria-labelledby="operation-log-title">
      ...
      <div class="log-page__table-wrap">
        <el-table v-loading="loading" :data="rows" row-key="id" empty-text="暂无系统操作日志">
      ...
      <div v-if="loading" class="log-page__mobile-state">正在加载系统操作日志</div>
      <div v-else-if="!rows.length" class="log-page__mobile-state">暂无系统操作日志</div>
