...
import { ArrowDown, ArrowRight, Delete, Refresh, VideoPlay } from '@element-plus/icons-vue'
...
const studentPanelCollapsed = ref(false)
const zonePanelCollapsed = ref(false)
...
const totalReservedRooms = computed(() => draft.zoneRows.reduce(
  (total, row) => total + Number(row.reservedEmptyRooms || 0),
  0,
))
...
function normalizeOption(row, kind) {
  const id = firstDefined(row, kind === 'college'
    ? ['id', 'collegeId', 'value']
    : ['id', 'campusId', 'zoneId', 'value'])
  const name = firstDefined(row, kind === 'college'
    ? ['collegeName', 'name', 'label', 'value']
    : [kind === 'campus' ? 'campusName' : 'zoneName', 'name', 'label', 'value'])
...
<template>
  <div class="bed-allocation-new-page">
    ...
    <main class="dashboard-stage">
      <section class="parameter-panel" aria-labelledby="student-parameter-title">
        ...
        <div class="panel-heading">
          ...
          <div class="panel-heading__tools">
            ...
            <el-button
              ...
              text
              circle
              :icon="studentPanelCollapsed ? ArrowRight : ArrowDown"
              :aria-label="studentPanelCollapsed ? '展开学生参数' : '收起学生参数'"
              :title="studentPanelCollapsed ? '展开学生参数' : '收起学生参数'"
              @click="studentPanelCollapsed = !studentPanelCollapsed"
            />
          </div>
        </div>

        <div v-if="!studentPanelCollapsed" class="table-scroll">
      ...
      <section class="parameter-panel" aria-labelledby="zone-parameter-title">
        ...
        <div class="panel-heading panel-heading--zone">
          ...
          <div class="panel-heading__tools">
            <div class="zone-toolbar">
              ...
              <el-button class="zone-refresh-button" :icon="Refresh" circle :loading="loading.zones" aria-label="刷新苑区列表" title="刷新苑区列表" @click="loadZones()" />
            </div>
            <div class="panel-stat"><strong>{{ totalReservedRooms }}</strong><span>预留空房间</span></div>
            <el-button
              class="collapse-button"
              text
              circle
              :icon="zonePanelCollapsed ? ArrowRight : ArrowDown"
              :aria-label="zonePanelCollapsed ? '展开苑区参数' : '收起苑区参数'"
              :title="zonePanelCollapsed ? '展开苑区参数' : '收起苑区参数'"
              @click="zonePanelCollapsed = !zonePanelCollapsed"
            />
          </div>
        </div>

        <div v-if="!zonePanelCollapsed" class="table-scroll">
        ...
        <div v-if="!zonePanelCollapsed" class="panel-footer">
          <el-button class="start-allocation-button" type="primary" :icon="VideoPlay" @click="startAllocation">开始排寝</el-button>
        </div>
        </div>
      </section>
    </main>
...
.header-action-button, .zone-refresh-button, .collapse-button { min-block-size: 2.125rem; border-color: rgba(147, 197, 253, .34); color: #dbeafe; background: rgba(5, 18, 38, .72); }
.header-action-button:hover, .header-action-button:focus-visible, .zone-refresh-button:hover, .zone-refresh-button:focus-visible, .collapse-button:hover, .collapse-button:focus-visible { border-color: #60a5fa; color: #fff; background: rgba(37, 99, 235, .45); }
.dashboard-stage { min-height: 0; flex: 1; overflow: auto; padding: clamp(.625rem, 1.2vw, 1.125rem); }
...
.panel-heading h2 { margin: 0; font-size: 1.125rem; }
...
.table-scroll { overflow-x: auto; padding-block-end: .125rem; }
.parameter-table { width: 100%; border-collapse: collapse; color: var(--screen-text); font-size: .8125rem; }
.panel-footer { display: flex; justify-content: flex-end; padding-block-start: clamp(.625rem, 1vw, .875rem); }
